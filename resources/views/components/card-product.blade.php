...
<div class="card card-side bg-white shadow-sm rounded-lg relative text-black mb-5">
    <figure class="w-[270px] h-[184px] overflow-hidden m-2 rounded-xl">
        <img src="{{ $image }}" alt="{{ $title }}"
            class="object-cover w-full h-full transition-transform duration-300 ease-in-out hover:scale-105" />
    </figure>

    <div
        class="absolute top-2 right-2 z-10 shadow-xl/15 bg-white rounded-[20px] w-9 h-9 flex items-center justify-center">
        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"
            class="size-5 text-gray-600 hover:text-green-500 transition-colors cursor-pointer">
            <path stroke-linecap="round" stroke-linejoin="round"
                d="M17.593 3.322c1.1.128 1.907 1.077 1.907 2.185V21L12 17.25 4.5 21V5.507c0-1.108.806-2.057 1.907-2.185a48.507 48.507 0 0 1 11.186 0Z" />
        </svg>
    </div>
    <div class="card-body pt-1.5 text-base gap-2.5">
        <h2 class="card-title text-xl w-[518px]">
            <a href="{{ $href }}" class="hover:underline">{{ $title }}</a>
        </h2>
        <div class="flex items-center">
            <div class="rating w-20 h-5 mr-1.5">
                @for ($i = 1; $i <= 5; $i++)
                    <input type="radio" name="rating-{{ $title }}" class="mask mask-star-2 bg-green-500"
                        aria-label="{{ $i }} star" @if ($i <= round($rating ?? 0)) checked @endif />
                @endfor
            </div>
            <span>
        ...
        <div class="flex items-center gap-1.5 w-[518px]">
        ...
        <div class="flex items-center gap-1.5" data-store-lat="{{ $latitude }}" data-store-lng="{{ $longitude }}">
            ...
            <span class="distance-badge">Đang tính khoảng cách...</span>
        </div>
        @endif
        <div class="flex items-center gap-1.5">
            <svg class="w-5 h-5" xmlns="http://www.w3.org/2000/svg"
                ...
                <span
                    class="{{ $status_label == 'Đang mở cửa' ? 'text-[#00b707]' : 'text-red-500' }} font-semibold">{{ $status_label }}</span>
