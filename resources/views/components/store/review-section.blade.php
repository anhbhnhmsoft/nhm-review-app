            ...
            @foreach ($reviews as $review)
                <div class="grid grid-cols-10 gap-4">
                        ...
                        @if ($review->user->avatar_path)
                            <div class="avatar">
                                <div class="w-full rounded-full">
                                    <img src="{{ \App\Utils\HelperFunction::generateURLImagePath($review->user->avatar_path) }}"
                        ...
                        @else
                            <div class="avatar avatar-placeholder w-full">
                                <div class="bg-gray-400 text-neutral-content w-full rounded-full">
                                    <span
                                        class="text-2xl uppercase">{{ \Illuminate\Support\Str::substr($review->user->name, 0, 1) }}</span>
                                </div>
                            </div>
                        @endif
                    </div>
                    <div class="card card-md col-span-8 lg:col-span-9 bg-gray-100 relative">
                        <div
                            class="absolute top-[30px] left-[-12px] w-0 h-0 border-l-[8px] border-l-transparent border-r-[8px] border-r-transparent border-b-[8px] border-b-gray-100 -translate-y-1/2 -rotate-90">
                        </div>
                            ...
                            <div class="flex items-center justify-between">
                                <div class="flex flex-col">
                                    ...
                                    <p class="text-sm text-gray-400">
                                        {{ \App\Utils\HelperFunction::humanReviewTime($review->created_at) }}</p>
                                </div>
                                {{-- Rating --}}
                                <div class="relative inline-block group">
                                    ...
                                    <!-- Popover -->
                                    <div class="absolute left-1/2 bottom-12 mt-2 -translate-x-1/2
                                    w-44 sm:w-56 md:w-64 max-w-[90vw]
                                    rounded-xl bg-white shadow-lg z-10 p-3
                                            ...
                                            <div class="flex flex-col">
                                                @foreach ($ratingLabel as $field => $label)
                                                    <div class="flex items-center gap-2 mb-2 flex-wrap">
                                                        <span
                                                            class="w-20 sm:w-24 text-xs sm:text-sm text-gray-600">{{ $label }}:</span>
                                ...
                                <div class="flex items-center gap-2 flex-wrap">
                                    @foreach ($review->reviewImages as $image)
                                        <a data-fancybox
                                            ...
                                            <img alt="Người dùng đánh giá"
                                                src="{{ \App\Utils\HelperFunction::generateURLImagePath($image->image_path) }}"
