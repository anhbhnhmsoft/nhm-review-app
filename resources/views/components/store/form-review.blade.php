<div x-data="{ open: @entangle('isOpen') }">
    <template x-teleport="body">
        <div x-transition:enter="transition ease-out duration-100" x-transition:enter-start="opacity-0"
            x-transition:enter-end="opacity-100" x-transition:leave-end="opacity-0" x-show="open"
            ...
            <div @click.away="$wire.close()"
                ...
                <form wire:submit.prevent="saveReview">
                            ...
                            <div class="flex-[2_1]">
                                <div class="rating justify-self-start">
                                    ...
                                    <input type="radio" name="rating_location" value="1" x-model="rating"
                                        class="mask mask-star-2 bg-green-600" aria-label="1 star" />
                                    <input type="radio" name="rating_location" value="2" x-model="rating"
                                        class="mask mask-star-2 bg-green-600" aria-label="2 star" />
                                    <input type="radio" name="rating_location" value="3" x-model="rating"
                                        class="mask mask-star-2 bg-green-600" aria-label="3 star" />
                                    <input type="radio" name="rating_location" value="4" x-model="rating"
                                        class="mask mask-star-2 bg-green-600" aria-label="4 star" />
                                    <input type="radio" name="rating_location" value="5" x-model="rating"
                                        class="mask mask-star-2 bg-green-600" aria-label="5 star" checked="checked" />
                                </div>
                            </div>
                            ...
                            <div class="flex-[2_1]">
                                <div class="rating justify-self-start">
                                    ...
                                    <input type="radio" name="rating_quality" value="5" x-model="rating"
                                        class="mask mask-star-2 bg-green-600" aria-label="5 star"
                                        checked="checked" />
                                </div>
                            </div>
                            <div class="flex-[1_1] justify-self-end">
                    ...
                    <div x-data="{ text: '', minLength: 10 }" class="mb-4">
                        <textarea x-model="text" name="review" wire:model="review" placeholder="Hãy nhập nội dung đánh giá"
                            class="textarea textarea-success w-full" rows="6"></textarea>
                        <div class="mt-2 text-end text-xs italic text-gray-400">
                            <span x-text="text.length"></span> kí tự (tối thiểu <span x-text="minLength"></span> ký
                            tự)
                        </div>
                        @error('review')
                    ...
                    <div class="flex justify-end gap-2">
                        <button type="button" @click="$wire.close()"
                            class="btn btn-primary-gray btn-sm">Đóng</button>
                        <button type="submit" class="btn btn-primary-green btn-sm flex items-center"
                            wire:loading.attr="disabled">
                            <span wire:loading.remove>Gửi đánh giá</span>
                            <span wire:loading>
                                <svg class="animate-spin size-3 text-white" xmlns="http://www.w3.org/2000/svg"
