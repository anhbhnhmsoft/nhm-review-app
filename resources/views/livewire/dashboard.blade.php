    ...
    <section class="d_section bg-white">
        ...
        <div class="flex mt-1 md:mt-12 items-center justify-center">
                <a href="{{ route('frontend.articles.news') }}"
                   class="btn btn-lg text-green-600 border border-green-600 rounded-md bg-white hover:bg-green-600 hover:text-white focus:outline-none focus:ring-2 focus:ring-green-600">Xem thêm</a>
            </div>
        </div>
    </section>
    <section class="d_section bg-slate-100">
        ...
        <h2 class="text-black text-lg mb-4">Các Video nổi bật hàng đầu của chúng tôi</h2>
            @if($featuredVideos && $featuredVideos->count() > 0)
                <div class="hidden lg:grid grid-cols-12 grid-rows-2 gap-2.5 auto-cols-auto">
                    @foreach($featuredVideos as $index => $store)
                        ...
                        @endphp
                        @if($video)
                            <div class="{{ $colSpan }} relative h-full overflow-hidden rounded-md group">
                                <div class="pb-[70%] h-full relative overflow-hidden">
                                    <img 
                                        src="{{ \App\Utils\HelperFunction::generateURLImagePath($store->logo_path) }}"
                                        alt="{{ $store->name }}"
                                        class="block absolute top-0 left-0 h-full w-full object-cover"
                                    />
                                </div>
                                <div class="absolute inset-0 bg-gradient-to-b from-transparent to-black/40 w-full flex flex-col justify-end">
                                    <div class="absolute inset-0 flex items-center justify-center cursor-pointer z-10">
                                        <div class="bg-red-500 opacity-90 h-[40px] w-[40px] rounded-full flex items-center justify-center transition-all duration-300 ease-in-out transform group-hover:scale-[2000%]">
                                        </div>
                                    </div>
                                    <a 
                                        href="{{ route('public_video', ['file_path' => $video->file_path]) }}"
                                        data-fancybox="featured-videos"
                                        data-poster="{{ \App\Utils\HelperFunction::generateURLImagePath($store->logo_path) }}"
                                        data-caption="{{ $store->name }} - {{ $store->address }}"
                                    ...
                                    <div class="p-2">
                                        ...
                                        <div class="flex items-center space-x-1">
                                            ...
                                            <span class="text-sm text-white max-h-16 truncate">{{ $store->address }}</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
                ...
                <div class="block lg:hidden mt-4">
                    <div class="swiper-container featured-videos-swiper !w-full !h-fit relative overflow-hidden">
                        <div class="swiper-wrapper py-4 !w-full !h-fit">
                            @foreach($featuredVideos as $store)
                                ...
                                @if($video)
                                    <div class="swiper-slide !h-fit">
                                        <div class="bg-transparent border border-[#e8e8e8] md:border-0 rounded-md transition-all duration-300 ease-in-out relative flex flex-col group md:min-h-[300px] shadow-sm md:shadow-none">
                                            <div class="relative pb-[60%] h-full overflow-hidden rounded-md">
                                                <img 
                                                    src="{{ \App\Utils\HelperFunction::generateURLImagePath($store->logo_path) }}"
                                                    alt="{{ $store->name }}"
                                                    class="block absolute top-0 left-0 h-full w-full object-cover"
                                                ...
                                                <div class="absolute inset-0 bg-gradient-to-b from-transparent to-black/40 w-full flex flex-col justify-end">
                                                    ...
                                                    <a 
                                                        href="{{ route('public_video', ['file_path' => $video->file_path]) }}"
                                                        data-fancybox="featured-videos-mobile"
                                                        data-poster="{{ \App\Utils\HelperFunction::generateURLImagePath($store->logo_path) }}"
                                                        data-caption="{{ $store->name }} - {{ $store->address }}"
                                                        class="absolute inset-0 flex flex-col items-center justify-center gap-2 z-20 group cursor-pointer"
                                                    >
                        ...
                        <div class="swiper-button-prev featured-videos-prev">
                            <i class="fa-solid fa-chevron-left text-gray-600"></i>
                        </div>
                        <div class="swiper-button-next featured-videos-next">
                            <i class="fa-solid fa-chevron-right text-gray-600"></i>
                        </div>
                    </div>
                </div>
