...
<main>
    ...
    <section class="d_section bg-slate-100">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2">
            <div class="flex flex-col">
                @if($pressArticle)
                    ...
                    <div class="inline-flex items-center gap-2 mb-4">
                        ...
                        <span class="font-medium text-gray-500">{{ $pressArticle->created_at->format('d/m/Y') }}</span>
                    </div>
                    <div class="my-4">
                        <p class="text-gray-500">{{ Str::limit(strip_tags($pressArticle->content), 200) }}</p>
                    </div>
                    <div class="">
                        <a href="{{ route('frontend.article-detail', $pressArticle->slug) }}" class="btn btn-lg bg-blue-600 hover:bg-blue-700 text-white rounded-xl">Xem thêm</a>
                    </div>
                @else
            ...
            @if($pressArticle && $pressArticle->image_path)
                <img class="hidden lg:block" src="{{ route('public_image', ['file_path' => $pressArticle->image_path]) }}" alt="{{ $pressArticle->title }}"/>
            @else
    ...
    <section class="d_section bg-white">
        ...
        @if($newsArticles->count() > 0)
            ...
            <div class="hidden lg:grid mt-4 grid-cols-12 grid-rows-2 auto-rows-fr gap-4">
                ...
                <div class="col-span-6 row-span-2 bg-transparent border-0 rounded-md transition-all duration-300 ease-in-out relative flex flex-col group">
                    <a href="{{ route('frontend.article-detail', $firstArticle->slug) }}" class="rounded-md h-full w-full block overflow-hidden !shrink-0 ">
                    ...
                    <div class="absolute bottom-0 left-0 right-0 p-6 bg-gradient-to-b from-transparent to-black/80 flex flex-col grow">
                ...
                @foreach($newsArticles->skip(1)->take(4) as $article)
                    <div class="col-span-3 row-span-1 bg-transparent border-0 group flex flex-col gap-2">
                        <a href="{{ route('frontend.article-detail', $article->slug) }}" class="rounded-md w-full block overflow-hidden !shrink-0 relative pb-[100%] h-auto">
                ...
                <div class="swiper-container news-swiper !w-full !h-fit overflow-hidden">
                    <div class="swiper-wrapper py-4 !w-full !h-fit">
                        @foreach($newsArticles as $article)
                            <div class="swiper-slide !h-fit">
                                <div class="bg-transparent border border-[#e8e8e8] md:border-0 rounded-md transition-all duration-300 ease-in-out relative flex flex-col group min-h-[300px] shadow-sm md:shadow-none">
                                    <a href="{{ route('frontend.article-detail', $article->slug) }}" class="rounded-md h-full w-full block overflow-hidden !shrink-0 relative pb-[60%]">
                                        @if($article->image_path)
                                            <img src="{{ route('public_image', ['file_path' => $article->image_path]) }}" 
                                                 alt="{{ $article->title }}"
                                    ...
                                    <div class="p-4 flex flex-col grow">
                                        ...
                                        <div class="inline-flex items-center gap-x-4 mt-auto">
                                            ...
                                            <div class="inline-flex items-center space-x-1">
                                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                                     stroke-width="1.5" stroke="currentColor" class="size-4 text-slate-600">
                                                    <path stroke-linecap="round" stroke-linejoin="round"
                                                          d="M2.036 12.322a1.012 1.012 0 0 1 0-.639C3.423 7.51 7.36 4.5 12 4.5c4.638 0 8.573 3.007 9.963 7.178.07.207.07.431 0 .639C20.577 16.49 16.64 19.5 12 19.5c-4.638 0-8.573-3.007-9.963-7.178Z"/>
                                                    <path stroke-linecap="round" stroke-linejoin="round"
                                                          d="M15 12a3 3 0 1 1-6 0 3 3 0 0 1 6 0Z"/>
        ...
        <div class="flex mt-1 md:mt-12 items-center justify-center">
            <a href="{{ route('frontend.articles.news') }}" class="btn btn-lg bg-blue-600 hover:bg-blue-700 text-white rounded-lg">Xem thêm</a>
            </div>
        </div>
    </section>
    <section class="d_section bg-slate-100">
    ...
    <section class="d_section bg-white">
        <div class="container pad_container">
            ...
            <div class="flex mt-1 md:mt-12 items-center justify-center">
                <a href="{{ route('frontend.articles.handbook') }}" class="btn btn-lg bg-blue-600 hover:bg-blue-700 text-white rounded-lg">Xem thêm</a>
            </div>
        </div>
    </section>
</main>
